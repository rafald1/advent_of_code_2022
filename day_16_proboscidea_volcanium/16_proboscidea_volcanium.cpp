...
int findMaxFlow(
    ...
    if (timeLeft <= 1 || checkIfCurrentValveIsAlreadyOpened(valves, currentValve, valveStatuses))
        return 0;

    std::string memoID = std::to_string(timeLeft) + currentValve + valveStatuses;
    ...
    for (size_t i{1}; i < valves.size(); ++i) {
        if (currentValve == valves[i])
            continue;

        int timeUsed{};

        if (currentValve != "AA") {
            openCurrentValveByUpdatingValveStatuses(valves, currentValve, valveStatuses);
            timeUsed = 1;
        }
        ...
        pressureReleased = std::max(
            ...
            findMaxFlow(valves, flowRates, distances, memo,
                        valves[i], timeLeft - timeUsed - distances.at({currentValve, valves[i]}), valveStatuses)
        );
    }

    memo[memoID] = pressureReleased + (timeLeft - 1) * flowRates.at(currentValve);
    return memo.at(memoID);
}
...
int main() {
    try {
        ...
        // processData("../test_input/16.txt", valves, flowRates, neighbours);  // 1651

        auto distances = calculateShortestPathsBetweenNodes(valves, neighbours);
        valves.clear();
        valves = findValvesAbleToReleasePressure(flowRates);
        std::unordered_map<std::string, int> memo{};
        auto result = findMaxFlow(valves, flowRates, distances, memo, "AA", 30, std::string(valves.size(), '0'));
