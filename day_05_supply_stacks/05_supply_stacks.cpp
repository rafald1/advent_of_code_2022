...
std::string rearrangeCrates(std::unordered_map<int, std::string>& stacksOfCrates,
    ...
    for (const auto& [numberOfCrates, fromStackNumber, toStackNumber] : rearrangementProcedures) {
        for (int i = 0; i < numberOfCrates; i++) {
            char crate = stacksOfCrates[fromStackNumber].back();
            stacksOfCrates[fromStackNumber].pop_back();
            stacksOfCrates[toStackNumber].push_back(crate);
        }
    }
...
int main() {
    try {
        ...
        // auto [crates, procedures] = processData("../test_input/05.txt");  // CMZ
