...
size_t pourSand(std::unordered_set<std::pair<int, int>>& cave) {
    ...
    for (const auto& rockPosition : cave) {
        if (rockPosition.second > deepestRock)
            deepestRock = rockPosition.second;
    }

    size_t floorLevel{2 + deepestRock};
    size_t numberOfRocks{cave.size()};
    std::pair<int, int> sandPosition{500, 0};
    auto& [sandPositionX, sandPositionY] = sandPosition;

    while (1) {
        if (floorLevel > sandPositionY + 1 && !cave.contains({sandPositionX, sandPositionY + 1})) {
            ++sandPositionY;
        } else if (floorLevel > sandPositionY + 1 && !cave.contains({sandPositionX - 1, sandPositionY + 1})) {
            --sandPositionX;
            ++sandPositionY;
        } else if (floorLevel > sandPositionY + 1 && !cave.contains({sandPositionX + 1, sandPositionY + 1})) {
            ++sandPositionX;
...
int main() {
    try {
        ...
        // std::vector<std::vector<std::pair<int, int>>> allPaths = processData("../test_input/14.txt");  // 93
