...
int simulate(
    ...
    if (canBotBeBuilt) {
        ...
        if (resources.clay <= botCost.at("obsidian").clay)
...
int useBlueprintsToSimulate(const std::vector<std::unordered_map<std::string, Resources>>& blueprints) {
    ...
    for (auto& blueprint : blueprints) {
        ...
        int sim1 = simulate(24, "ore", blueprint, botCount, startingResources);
        int sim2 = simulate(24, "clay", blueprint, botCount, startingResources);
        geodeCount += i * std::max(sim1, sim2);
        ++i;
    }
...
int main() {
    try {
        ...
        // auto allBlueprints = processData("../test_input/19.txt");  // 33
