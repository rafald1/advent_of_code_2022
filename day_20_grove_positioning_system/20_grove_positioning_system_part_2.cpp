...
void applyDecryptionKLey(std::vector<IndexedValue>& numbers, const long& decryptionKey) {
...
long calculateGroveCoordinates(const std::vector<IndexedValue>& numbers) {
    size_t numSize{numbers.size()};
...
int main() {
    try {
        ...
        // auto recievedNumbers = processData("../test_input/20.txt");  // 1623178306

        applyDecryptionKLey(recievedNumbers, 811589153);
        decrypt(recievedNumbers);
        long result = calculateGroveCoordinates(recievedNumbers);
